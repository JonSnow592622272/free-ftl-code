...
<mapper namespace="${diy_mapperJava_targetPackage}.${introspectedTable.tableConfiguration.properties.module}.mapper.${tuofengTableName?cap_first}Mapper">

        <#--生成主键排在第一位，非主键排在后面，这里缩为一行是为了避免替换之后出现换行问题，注意！！！fileCreateType=2时,第一次生成会出现未换行的问题，再次生成一次即可恢复正常-->
    <resultMap id="BaseResultMap" type="${diy_model_targetPackage}.${introspectedTable.tableConfiguration.properties.module}.entity.${tuofengTableName?cap_first}"><#list introspectedTable.primaryKeyColumns as primaryKey><id column="${primaryKey.actualColumnName}" jdbcType="${primaryKey.jdbcTypeName}" property="${primaryKey.javaProperty}"/></#list><#list introspectedTable.nonPrimaryKeyColumns as baseColumns><result column="${baseColumns.actualColumnName}" jdbcType="${baseColumns.jdbcTypeName}" property="${baseColumns.javaProperty}"/></#list></resultMap>
