...
<mapper namespace="com.shotgun.my.haha.${tuofengTableName?cap_first}Mapper">

        <#--生成主键排在第一位，非主键排在后面，这里缩为一行是为了避免替换之后出现换行问题-->
    <resultMap id="BaseResultMap" type="com.shotgun.my.haha.${tuofengTableName?cap_first}"><#list introspectedTable.primaryKeyColumns as primaryKey><id column="${primaryKey.actualColumnName}" jdbcType="${primaryKey.jdbcTypeName}" property="${primaryKey.javaProperty}"/></#list><#list introspectedTable.nonPrimaryKeyColumns as baseColumns><result column="${baseColumns.actualColumnName}" jdbcType="${baseColumns.jdbcTypeName}" property="${baseColumns.javaProperty}"/></#list></resultMap>
